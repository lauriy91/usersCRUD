...
import { UserDTO } from 'src/users/parameters/user.dto';
...
describe('UsersController (e2e)', () => {
  ...
  it('users CRUD', async () => {
    ...
    const newUserRequest = await server
      ...
      .send(newUser)
      .expect(201);
    expect(newUserRequest.body.name).toBe(newUser.full_name);
    ...
    //   .type('form')
    //   .send(updateUser);
    // expect(updateUserRequest.body.name).toEqual(updateUser.full_name);
